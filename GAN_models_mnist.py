...
def discriminator():
    model = Sequential()
    #1->(32,32,128)
    model.add(Convolution2D(32, (3, 3), strides=(1, 1), input_shape=(28, 28, 1), padding='same'))
    model.add(BatchNormalization())
    model.add(LeakyReLU(0.2))
    # model.add(Dropout(0.2))
    #2->(16,16,256)
...
def generator(inputdim = 20, xdim=7, ydim=7):
    model = Sequential()
    #pre, 100->1024*4*4
    model.add(Dense(input_dim=inputdim, units=64 * xdim * ydim))
    #1)4*4*1024->8*8*512
    # model.add(BatchNormalization())#batch norm in G can cause strong intra-class correlation
    model.add(Activation('relu'))
    model.add(Reshape((xdim, ydim,64), input_shape=(inputdim,)))
    model.add(UpSampling2D(size=(2, 2)))
    model.add(Convolution2D(64, (3, 3), padding='same'))
    #2)->16*16*256
    # model.add(BatchNormalization())
